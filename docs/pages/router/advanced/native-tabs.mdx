...
> **important** Native tabs are an experimental feature available in SDK 54+, and their API is subject to change.

Tabs are a common way to navigate between different sections of an app. In Expo Router, you can use different tab layouts, depending on your needs. This page covers the native tabs. Unlike the [other tabs layout](/router/advanced/tabs), native tabs use the native system tab bar.
...
> **info** The custom images API is currently not available on Android, but we plan to add it to soon.
